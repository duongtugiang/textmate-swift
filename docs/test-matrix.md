...
| `regexp` | 8 | 41 | Dispositioned | 4.T1 | [~] | **Dispositioned (41/41):** 4.T2 spike chose ICU emulation over an Oniguruma port — `t_find` (2)/`t_match` (1) test Onigmo internals now covered by the grammar suites (`t_anchors`/`t_begin_while`/`t_capture_rules` + `TextRegex` anchor-emulation tests) at the engine level; `t_format_string` (7)/`t_snippet` (8) need the snippet/format-string frameworks (4.S5); `t_glob` (13)/`t_glob_list` (3) are bundle/file matching (4.S6); `t_indent` (6) is a text utility ported when the indentation features land; `t_escape` (1) is exercised by the emulation layer. Tracked, never silently dropped |
...
| `bundles` | 2 | 5 | Port now | 4.T3 | [ ] | Depends on grammar-engine spike 4.T2 (done) + bundle loading (4.S6); 4.T3 remains open |
...
| 4 | 58/58 (regexp + scope + parse) + 5/5 (bundles) | ✅ scope 13/13 + parse 4/4 green (17/58; +7 Swift tests → **102 total**); regexp 41/41 dispositioned above (ICU emulation supersedes Onigmo internals; utility suites follow 4.S5/4.S6); bundles 5/5 pending 4.T3 |
